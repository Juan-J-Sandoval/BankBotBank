...
def lambda_handler(event, context):
    ...
    if operation == 'get':
        result = get(event['payload'], payload_dictionary)
        return result
    
    if operation == 'update':
        result = update(event['payload'], payload_dictionary)
        return result

    if operation == 'add':
        print(payload_dictionary)
        result = add(event['payload'], payload_dictionary)
        return result
...
def get(item, diccionary):
    training_data_file_path = "/tmp/responseDatas.json"
    print("descargando training data a {}".format(training_data_file_path))
    s3.meta.client.download_file(bucket_name, "responseDatas.json", training_data_file_path)
    print("modelo de lectura en{}".format(training_data_file_path))
    try:
        with io.open(training_data_file_path) as f:
            trainingdata = json.load(f)
            print("tipo de variable")
            print(type(trainingdata))
            result = {
                'statusCode': 200,
                'payload': trainingdata['ResponseData']
            }
...
def update(item, diccionary):
    ...
    with io.open(training_data_file_path) as f:
        trainingdata = json.load(f)
        print("old data")
        print(trainingdata["ResponseData"])
    print("new data")
    print(item)
    print("guardando modelo")
    s3.meta.client.put_object(Bucket=bucket_name,Key="responseDatas.json",Body=trainingdata)
    return {"statusCode": 200}

def add(item, diccionary):
    if item['lexemas'] != "":
        vector=vectorizeText(item['lexemas'])
        result = {
            "statusCode": 200,
            "payload": "lexemas almacenados"
        }
    else:
        result = {
            "statusCode": 200,
            "payload": "faltan datos"
        }
    return result
